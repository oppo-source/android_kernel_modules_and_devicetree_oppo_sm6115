...
bool lcd_cabc_flag = true;
EXPORT_SYMBOL(lcd_cabc_flag);
		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_ON:
		...
		if (!strcmp(display->panel->name, "AC190 P 4 A0013 video mode dsi panel") ||
		    !strcmp(display->panel->name, "AC190 P D A0018 video mode dsi panel") ||
		    !strcmp(display->panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
		    LCD_INFO("[oplus] dsi_display->panel->name:%s\n", display->panel->name);
		    lcd_cabc_flag = true;
		}
