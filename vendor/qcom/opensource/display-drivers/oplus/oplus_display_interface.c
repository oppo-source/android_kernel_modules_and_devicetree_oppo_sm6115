				...
				struct dsi_parser_utils *utils)
{
	...
	rc = utils->read_u32(utils->data, "oplus,apollo-sync-backlight-debounce",
				  &priv_info->sync_bl_debounce);
	if (rc) {
		LCD_DEBUG("apollo sync backlight debounce time wasn't configed, set to default 1200us. rc=%d\n", rc);
		priv_info->sync_bl_debounce = 1200;
	} else {
		if(priv_info->sync_bl_debounce >= priv_info->vsync_period) {
			LCD_ERR("apollo sync backlight debounce time was out of vsync period, set to default 1200us.\n");
			priv_info->sync_bl_debounce = 1200;
		}
	}
