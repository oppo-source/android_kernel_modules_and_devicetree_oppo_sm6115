		...
		struct drm_encoder *drm_enc, bool enable)
{
	...
	if (!dsi_display || !dsi_display->panel || !dsi_display->panel->cur_mode
		...
		SDE_ERROR("sde encoder Invalid params(s) dsi_display %pK, panel %pK\n",
			dsi_display,
	...
	if ((rsc_config->fps != mode_info->frame_rate) ||
	    ...
	    if (!strcmp(dsi_display->panel->name, "AC190 P 4 A0013 video mode dsi panel") ||
		    !strcmp(dsi_display->panel->name, "AC190 P D A0018 video mode dsi panel") ||
		    !strcmp(dsi_display->panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
			SDE_DEBUG("dsi_display->panel->name:%s\n", dsi_display->panel->name);
			if ((rsc_config->fps != mode_info->frame_rate) &&
