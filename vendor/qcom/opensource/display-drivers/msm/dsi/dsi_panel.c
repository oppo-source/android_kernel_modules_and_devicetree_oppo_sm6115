...
#ifdef OPLUS_FEATURE_DISPLAY
#if IS_ENABLED(CONFIG_OCP2130_BIAS)
...
static int tp_irq = 0;
/* Only use to send DRM_PANEL_EVENT_FOR_TOUCH event, and you can user "panel->name" to judge primary/secondary panel */
#define OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(tp_index, data) { \
	int blank = data; \
	...
	notification.notif_type = DRM_PANEL_EVENT_FOR_TOUCH; \
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (!strcmp(panel->name, "AC166 P D A0018 video mode dsi panel")) {
		DSI_INFO("[%s][TP] notify touch driver to notify tp reset high.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x11); //LCD_CTL_TP_LOAD_FW
	}

	if (!strcmp(panel->name, "AC190 P D A0018 video mode dsi panel") ||
		!strcmp(panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
		DSI_INFO("[%s][TP] AC190 P D notify touch driver to notify tp reset high.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x11); //LCD_CTL_TP_RST_HIGH
	}

	if (gpio_is_valid(panel->panel_vci_gpio)) {
		rc = gpio_direction_output(panel->panel_vci_gpio, 1);
		if (rc)
			DSI_ERR("unable to set dir for panel_vci_gpio rc=%d", rc);
		gpio_set_value(panel->panel_vci_gpio, 1);
	}

	if (!strcmp(panel->name,"AC166 P 3 A0013 video mode dsi panel")
		||  !strcmp(panel->name, "AC166 P D A0018 video mode dsi panel")) {
		lcd_bias_set_vspn(1, 0, 6000);
	}

#ifdef OPLUS_FEATURE_DISPLAY
#if IS_ENABLED(CONFIG_OCP2130_BIAS)
	if (!strcmp(panel->name, "AC190 P 4 A0013 video mode dsi panel") ||
		!strcmp(panel->name, "AC190 P D A0018 video mode dsi panel") ||
		!strcmp(panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
		DSI_DEBUG("ocp2130_enable panel power on\n");
		ocp2130_enable();
	}
#endif
#endif
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
if (!strcmp(panel->name, "AC166 P 3 A0013 video mode dsi panel") \
	|| !strcmp(panel->name, "oplus22361 ili9883c 90hz boe hd video mode dsi panel") \
	|| !strcmp(panel->name, "AC166 P D A0018 video mode dsi panel")) {
	DSI_INFO("[%s][TP] notify touch driver to notify tp load firmware.\n", __func__);
	OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x10); //LCD_CTL_TP_LOAD_FW
}
/*#endif OPLUS_FEATURE_TP_BASIC*/
if (!strcmp(panel->name, "AC190 P 4 A0013 video mode dsi panel") \
	|| !strcmp(panel->name, "AC190 P D A0018 video mode dsi panel") \
...
if (!strcmp(panel->name, "AC166 P 3 A0013 video mode dsi panel") \
		|| !strcmp(panel->name, "oplus22361 ili9883c 90hz boe hd video mode dsi panel")
		|| !strcmp(panel->name, "AC166 P D A0018 video mode dsi panel")
		|| !strcmp(panel->name, "AC190 P 4 A0013 video mode dsi panel")
		|| !strcmp(panel->name, "AC190 P D A0018 video mode dsi panel")
		|| !strcmp(panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
...
/*#endif OPLUS_FEATURE_TP_BASIC*/
	...
	if (!strcmp(panel->name, "AC190 P 4 A0013 video mode dsi panel") ||
		...
		!strcmp(panel->name, "AC190 P D 1 A0018 video mode dsi panel")) {
		if((is_pd_with_guesture == false) || (1 == shutdown_flag) || panel_esd_check_failed) {
			DSI_DEBUG("ocp2130_disable panel power off\n");
			ocp2130_disable();
		}
	}
#endif
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
	/*if ((is_pd_with_guesture == false) || (1 == shutdown_flag) || panel_esd_check_failed) {*/
	if (is_pd_with_guesture == false)
	{
		DSI_INFO("[%s][TP] MTC_LOG: notify touch driver to pull up cs gpio when screen off.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x19); //LCD_CTL_CS_ON
	}
	if ((1 == shutdown_flag) || panel_esd_check_failed) {
		DSI_INFO("[%s][TP] notify touch driver to pull down cs gpio.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x1A); //LCD_CTL_CS_OFF
	}
	if (!strcmp(panel->name, "AC166 P D A0018 video mode dsi panel")) {
		DSI_INFO("[%s][TP] notify touch driver to notify tp reset low.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x12); //LCD_CTL_TP_LOAD_FW
	}

	if (gpio_is_valid(panel->reset_config.disp_en_gpio))
		gpio_set_value(panel->reset_config.disp_en_gpio, 0);

	if (!strcmp(panel->name, "AC166 P D A0018 video mode dsi panel") && (true != is_pd_with_guesture)) {
		DSI_INFO("[%s][TP] notify touch driver to notify tp reset high.\n", __func__);
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x11); //LCD_CTL_RST_ON
	}
	...
	} else if (!strcmp(panel->name, "AC166 P D A0018 video mode dsi panel") && (true == is_pd_with_guesture)) {
		if (gpio_is_valid(panel->reset_config.reset_gpio) &&
					!panel->reset_gpio_always_on)
			DSI_INFO("[%s][TP] pull up lcd reset when enable screen off gesture for TD.\n", __func__);
			gpio_set_value(panel->reset_config.reset_gpio, 1);
	} else if (!strcmp(panel->name, "AC190 P 4 A0013 video mode dsi panel")) {
		if (gpio_is_valid(panel->reset_config.reset_gpio) &&
					!panel->reset_gpio_always_on)
			gpio_set_value(panel->reset_config.reset_gpio, 1);
	} else if ((!strcmp(panel->name, "AC190 P D A0018 video mode dsi panel") ||
			    !strcmp(panel->name, "AC190 P D 1 A0018 video mode dsi panel")) && (is_pd_with_guesture == true)) {
		if (gpio_is_valid(panel->reset_config.reset_gpio) &&
					!panel->reset_gpio_always_on)
			gpio_set_value(panel->reset_config.reset_gpio, 1);
			DSI_INFO("[%s][TP] pull up lcd reset when enable screen off gesture for TD.\n", __func__);
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	panel->panel_vci_gpio = utils->get_named_gpio(utils->data,
					"qcom,platform-panel-vci-gpio",
					0);
	if (!gpio_is_valid(panel->panel_vci_gpio))
		DSI_DEBUG("%s:%d qcom,platform-panel-vci-gpio\n", __func__,
			 __LINE__);

	panel->panel_vddr_gpio = utils->get_named_gpio(utils->data,
					"qcom,platform-panel-vddr-gpio",
					0);
	if (!gpio_is_valid(panel->panel_vddr_gpio))
		DSI_DEBUG("%s:%d qcom,platform-panel-vddr-gpio\n", __func__,
			 __LINE__);
...
/*#ifdef OPLUS_FEATURE_TP_BASIC*/
	//OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x1B); //LCD_CTL_PRE_DISABLE
/*#endif OPLUS_FEATURE_TP_BASIC*/
...
if (!strcmp(panel->name, "AC166 P 3 A0013 video mode dsi panel") \
	...
	if ((tp_gesture_enable_notifier && (1 != tp_gesture_enable_notifier(0))) || (1 == shutdown_flag)) {
		...
		OPLUS_PANEL_EVENT_NOTIFY_FOR_TOUCH(0, 0x12); //LCD_CTL_RST_OFF
	}
}
/*#endif OPLUS_FEATURE_TP_BASIC*/
