...
int cam_soc_util_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
	...
	rc = of_property_read_string_index(of_node, "flash-name",
				0, &(soc_info->flash_name));
	if (rc) {
		CAM_DBG(CAM_UTIL, "reading flash_name failed");
	} else {
		CAM_INFO(CAM_UTIL, "reading flash_name = %s", soc_info->flash_name);
	}
