	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
		...
		} else if ((eb_info->eeprom_name != NULL) && (!strcmp(eb_info->eeprom_name, "avatarb4_sc520cs")) && (0 <= get_insensor_eeprom_index(e_ctrl))) {
				rc = read_insensor_eeprom_data(e_ctrl, memptr);
				if (rc < 0){
					CAM_ERR(CAM_EEPROM, "insensor eeprom read failed  %d",rc);
					return rc;
				} else {
					CAM_INFO(CAM_EEPROM, "insensor eeprom read success");
				}
		} else {
			if (emap[j].mem.valid_size) {
				rc = camera_io_dev_read_seq(&e_ctrl->io_master_info,
