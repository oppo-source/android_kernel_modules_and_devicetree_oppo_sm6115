...
{
	int32_t rc = 0, i = 0, retry_count = 0;
	struct cam_flash_ctrl *fctrl     = NULL;
	...
	if (of_find_property(pdev->dev.of_node, "cci-master", NULL)) {
		...
		if ((NULL != fctrl->soc_info.flash_name) && (!strncmp(fctrl->soc_info.flash_name, "sgm3785", sizeof("sgm3785"))))
		{
			fctrl->func_tbl.parser = cam_flash_gpio_pkt_parser_sgm3785;
			fctrl->func_tbl.apply_setting = cam_flash_gpio_apply_setting_sgm3785;
			fctrl->func_tbl.power_ops = cam_flash_gpio_power_ops_sgm3785;
			fctrl->func_tbl.flush_req = cam_flash_gpio_flush_request_sgm3785;
			CAM_ERR(CAM_FLASH,"flash_name is %s", fctrl->soc_info.flash_name);
		} else {
	...
	//find exit flash device
	if (fctrl->flash_type != 2){
		for (i = 0; i < flash_ftm_data.total_flash_dev; i++) {
			if ((NULL != soc_info->flash_name) && (0 != strcmp(soc_info->flash_name, flash_ftm_data.flash_ftm_settings[i].flashprobeinfo.flash_name))) {
				CAM_INFO(CAM_FLASH, "Skip probe flash %s !", flash_ftm_data.flash_ftm_settings[i].flashprobeinfo.flash_name);
				continue;
			}
	...
	CAM_ERR(CAM_FLASH, "swami Probe success");
	return rc;
	...
	const struct i2c_device_id *id)
{
	...
	fctrl->flash_state = CAM_FLASH_STATE_INIT;
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
    rc = oplus_iic_flash_ftm(fctrl,client);
	#endif

	CAM_ERR(CAM_FLASH, "I2c flash swami probe sucess");
	return rc;
