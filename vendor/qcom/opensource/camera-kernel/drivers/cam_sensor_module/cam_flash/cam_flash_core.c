...
int cam_flash_off(struct cam_flash_ctrl *flash_ctrl)
{
	...
	if (flash_ctrl->flash_type == 2) {
		if (!strncmp(flash_ctrl->soc_info.flash_name, "sgm3785", sizeof("sgm3785"))) {
			cam_flash_gpio_off_sgm3785(flash_ctrl);
		} else {
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		cam_flash_check_boost_state(fctrl,CAM_FLASH_BOOST_PARSE|CAM_FLASH_BOOST_ON,
												csl_packet->header.request_id);
#endif
...
int cam_flash_publish_dev_info(struct cam_req_mgr_device_info *info)
{
	...
	{
		info->p_delay = CAM_I2C_FLASH_PIPELINE_DELAY;
	} else {
		info->p_delay = CAM_FLASH_PIPELINE_DELAY;
	}
	#endif
