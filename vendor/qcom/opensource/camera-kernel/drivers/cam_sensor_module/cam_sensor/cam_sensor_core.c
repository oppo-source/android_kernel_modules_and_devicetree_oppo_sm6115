	...
	int32_t cmd_buf_num, uint32_t cmd_buf_length, size_t remain_len)
{
	...
	case 0: {
		...
		CAM_ERR(CAM_SENSOR, "swami remain_len = %d and struct size = %d+%d", remain_len, sizeof(struct cam_cmd_i2c_info), sizeof(struct cam_cmd_probe));
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	#ifdef OPLUS_FEATURE_CAMERA_COMMON
	if (s_ctrl->sensordata->slave_info.flash_id != 0) {
	    rc = oplus_match_flash(s_ctrl);
	    if(rc < 0)
	    {
	        CAM_ERR(CAM_SENSOR, "oplus_match_flash failed");
	        return -ENODEV;
	    }
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
        ...
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF)
        {
            ...
            if(rc < 0) {
                CAM_ERR(CAM_SENSOR, "swami sensor power up faild!");
                mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
                goto free_power_settings;
             } else {
                CAM_ERR(CAM_SENSOR, "swami sensor power up success sensor id 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
                s_ctrl->sensor_power_state = CAM_SENSOR_POWER_ON;
             }
        } else {
            CAM_ERR(CAM_SENSOR, "swami sensor have power up!");
        }
        mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
#else
		...
		if (rc < 0) {
            ...
            mutex_lock(&(s_ctrl->sensor_power_state_mutex));
            cam_sensor_power_down(s_ctrl);
            CAM_INFO(CAM_SENSOR, "swami sensor power down!");
            msleep(20);
            s_ctrl->sensor_power_state = CAM_SENSOR_POWER_OFF;
            mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
            KEVENT_FB_SNESOR_PROBE_FAILED(fb_payload, "swami sensor match failed", s_ctrl->sensordata->slave_info.sensor_id);
            goto free_power_settings;
#else
			cam_sensor_power_down(s_ctrl);
			msleep(20);
        ...
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_ON)
        {
            ...
            if(rc < 0) {
                CAM_ERR(CAM_SENSOR, "sensor power down faild!");
                mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
                KEVENT_FB_SNESOR_PROBE_FAILED(fb_payload, "sensor power down failed", s_ctrl->sensordata->slave_info.sensor_id);
                goto free_power_settings;
             } else {
	...
	case CAM_ACQUIRE_DEV: {
		...
		CAM_DBG(CAM_SENSOR,"oplus_chg_set_camera success");
#endif
		if (s_ctrl->bridge_intf.device_hdl != -1) {
        ...
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF)
        {
            ...
            if(rc < 0) {
                ...
                CAM_ERR(CAM_SENSOR, "swami sensor power up success sensor id 0x%x",
                    s_ctrl->sensordata->slave_info.sensor_id);
                s_ctrl->sensor_power_state = CAM_SENSOR_POWER_ON;
             }
        } else {
	...
	case CAM_RELEASE_DEV: {
		...
		CAM_DBG(CAM_SENSOR, "oplus_chg_set_camera failed");
#endif
#ifdef OPLUS_FEATURE_CAMERA_COMMON
        mutex_lock(&(s_ctrl->sensor_power_state_mutex));
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_ON)
	...
	case CAM_GET_DPC_DATA: {
        rc = oplus_get_dpc_data(s_ctrl,arg);
        if(rc < 0)
        {
            CAM_ERR(CAM_SENSOR,"Failed Copy to User");
            goto release_mutex;
	...
	case CAM_OEM_GET_ID : {
		...
		CAM_DBG(CAM_SENSOR, "Valid Opcode: %d, index %d", cmd->op_code, s_ctrl->soc_info.index);
		break;
	}
#endif
	default:
