...
int32_t cam_qup_i2c_read_smbus(struct i2c_client *client,
	uint32_t addr, uint32_t *data,
	enum camera_sensor_i2c_type addr_type,
	enum camera_sensor_i2c_type data_type)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "i2c read failed slave addr 0x%x reg 0x%x",client->addr,addr);
	} else {
		*data = (u8) rc;
		rc = 0;
	}
	CAM_DBG(CAM_SENSOR, "i2c slave addr 0x%x reg 0x%x data 0x%x rc %d",client->addr,addr,*data,rc);
	return rc;
}

int32_t cam_qup_i2c_write_table_smbus(struct camera_io_master *client,
	struct cam_sensor_i2c_reg_setting *write_setting)
{
	int i;
	int32_t rc = -EINVAL;
	...
	for (i = 0; i < write_setting->size; i++) {
		...
		rc = i2c_smbus_write_byte_data(client->client, reg_setting->reg_addr, reg_setting->reg_data);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "i2c smbus write error slave addr 0x%x reg: 0x%x, data: 0x%x",
									client->client->addr, reg_setting->reg_addr, reg_setting->reg_data);
			break;
		}
		reg_setting++;
	}
