		...
		uint64_t req_id)
{
		...
		struct cam_flash_frame_setting *flash_data = NULL;
		CAM_DBG(CAM_FLASH, "cam_flash_gpio req_id =%llu",req_id);
		if (req_id == 0) {
			if (fctrl->nrt_info.cmn_attr.cmd_type ==
				CAMERA_SENSOR_FLASH_CMD_TYPE_INIT_FIRE) {
				flash_data = &fctrl->nrt_info;
				CAM_DBG(CAM_REQ,
					"FLASH_INIT_FIRE req_id: %llu flash_opcode: %d",
				...
				flash_data = &fctrl->nrt_info;
				CAM_DBG(CAM_REQ,
					"FLASH_WIDGET req_id: %llu flash_opcode: %d",
					req_id, flash_data->opcode);
				...
				CAM_DBG(CAM_REQ, "FLASH_RER req_id: %llu", req_id);
			...
			frame_offset = req_id % MAX_PER_FRAME_ARRAY;
			flash_data = &fctrl->per_frame[frame_offset];
			CAM_DBG(CAM_REQ, "FLASH_RT req_id: %llu flash_opcode: %d",
				req_id, flash_data->opcode);
