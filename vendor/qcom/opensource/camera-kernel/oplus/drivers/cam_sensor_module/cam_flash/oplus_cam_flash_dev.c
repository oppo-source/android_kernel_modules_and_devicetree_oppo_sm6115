...
void cam_flash_check_boost_state(struct cam_flash_ctrl *fctrl,int boost_ctrl,uint64_t req_id)
{
	...
	struct cam_sensor_i2c_reg_array * reg_setting = NULL, * flash_reg_setting = NULL ;
	struct cam_flash_ftm_settings *cur_flash = &flash_ftm_data.flash_ftm_settings[flash_ftm_data.valid_setting_index];
	struct cam_flash_ftm_reg_setting *off_setting = &(cur_flash->flashoffsettings);
	struct cam_flash_ftm_reg_setting *high_setting = &(cur_flash->flashhighsettings);
	int frame_offset = 0,i = 0;
	int do_boost = false;
	int boost_state = boost_ctrl & 0xFF;
	int boost_type  = boost_ctrl & 0xFF00;
	if(flash_ftm_data.flash_ftm_settings[flash_ftm_data.valid_setting_index].need_boost_check){
		if(boost_type == CAM_FLASH_BOOST_FORCE){
			do_boost = true;
		}else{
			/* RealTime */
			...
			if ((i2c_set->is_settings_valid == true) &&
				...
				list_for_each_entry(i2c_list,
					...
					if(write_setting && write_setting->reg_setting && high_setting && off_setting){
						if(boost_state == CAM_FLASH_BOOST_ON){
							flash_reg_setting = high_setting->reg_setting;
						}else{
							flash_reg_setting = off_setting->reg_setting;
						}
						reg_setting = write_setting->reg_setting;
						for (i = 0; i < write_setting->size; i ++) {
							if ((reg_setting->reg_addr == flash_reg_setting->reg_addr) && \
										(reg_setting->reg_data == flash_reg_setting->reg_data)){
								do_boost = true;
								break;
							}
							reg_setting++;
		...
		if(do_boost){
			if(boost_state != fctrl->cur_boost_state){
				CAM_INFO(CAM_FLASH, "set flash boot type: 0x%x state %d -> %d",boost_type,fctrl->cur_boost_state,boost_state);
				oplus_chg_set_camera_on(boost_state);
				fctrl->cur_boost_state = boost_state;
			}
		}
	}
}
...
int cam_flash_check_boost_i2c_apply_setting(struct cam_flash_ctrl *fctrl,uint64_t req_id)
{
	int rc = 0;
	if (req_id == 0) {
		return cam_flash_i2c_apply_setting(fctrl,req_id);
	} else {
		cam_flash_check_boost_state(fctrl,CAM_FLASH_BOOST_APPLY | CAM_FLASH_BOOST_ON,req_id);
		rc = cam_flash_i2c_apply_setting(fctrl,req_id);
		cam_flash_check_boost_state(fctrl,CAM_FLASH_BOOST_APPLY | CAM_FLASH_BOOST_OFF,req_id);
	}
	return rc;
}
...
int cam_ftm_i2c_flash_off(struct cam_flash_ctrl *flash_ctrl, struct cam_flash_ftm_settings *flash_ftm_data)
{
	...
	cam_flash_check_boost_state(flash_ctrl,CAM_FLASH_BOOST_FORCE | CAM_FLASH_BOOST_OFF,0);
	//2.power off
...
int cam_ftm_i2c_torch_on(struct cam_flash_ctrl *flash_ctrl, struct cam_flash_ftm_settings *flash_ftm_settings)
{
	int rc = 0, i = 0;
	struct cam_flash_ftm_power_setting *power_on_setting = &(flash_ftm_settings->flashpowerupsetting);
	...
	if (flash_ctrl->io_master_info.master_type == I2C_MASTER) {
		...
		flash_ctrl->io_master_info.cci_client->sid = flash_ftm_settings->cci_client.sid >> 1;
	}
...
int cam_ftm_i2c_flash_on(struct cam_flash_ctrl *flash_ctrl, struct cam_flash_ftm_settings *flash_ftm_data)
{
	...
	cam_flash_check_boost_state(flash_ctrl,CAM_FLASH_BOOST_FORCE | CAM_FLASH_BOOST_ON,0);
...
int isProjMatch(uint32_t *projID)
{
	...
	if(projID[index] == 0)
	{
		CAM_INFO(CAM_FLASH,"No project id information, skip match projID[0] 0x%x",projID[index]);
		return 1;
	}

	for(index = 0;index < MAX_PROJID_NUM;index ++ )
	{
		if(projID[index] != 0 && projID[index] == get_project())
		{
			CAM_INFO(CAM_FLASH,"match project id %d",projID[index]);
			return 1;
		}
	}
	CAM_ERR(CAM_FLASH,"match project id error");
	return 0;
}
...
int cam_flash_match_id(uint32_t except_id, struct cam_flash_ctrl *flash_ctrl, struct cam_flash_ftm_settings *flash_ftm_data)
{
	...
	if (flash_ctrl->io_master_info.master_type == I2C_MASTER) {
		flash_ctrl->io_master_info.client->addr = flash_ftm_data->flashprobeinfo.slave_write_address;
		flash_ctrl->io_master_info.use_i2c_smbus = flash_ftm_data->flashprobeinfo.use_i2c_smbus;
		if(flash_ftm_data->flashprobeinfo.chip_id_slave_address != 0){
			flash_ctrl->io_master_info.client->addr = flash_ftm_data->flashprobeinfo.chip_id_slave_address;
		}
	} else if(flash_ctrl->io_master_info.master_type == CCI_MASTER) {
		flash_ctrl->io_master_info.cci_client->cci_i2c_master = flash_ftm_data->cci_client.cci_i2c_master;
		flash_ctrl->io_master_info.cci_client->i2c_freq_mode = flash_ftm_data->cci_client.i2c_freq_mode;
	...
	if(match_flag ==1 && 1 == isProjMatch(flash_ftm_data->flashprobeinfo.projID))
	{
		match_flag = 1;
	}else{
		match_flag = 0;
	...
	flash_ctrl->io_master_info.client->addr = flash_ftm_data->flashprobeinfo.slave_write_address;
	flash_ctrl->cur_boost_state = 0;
	if(flash_ftm_data->apply_setting != NULL){
		flash_ctrl->func_tbl.apply_setting = flash_ftm_data->apply_setting;
	}
						...
						size_t len, loff_t *data)
{
	...
	buf[len] =  '\0';
	mutex_lock(&(vendor_flash_ctrl->ftm_flash_lock));
	...
	mutex_unlock(&(vendor_flash_ctrl->ftm_flash_lock));
	return len;
}
...
int flash_proc_init(struct cam_flash_ctrl *flash_ctrl)
{
	...
	CAM_INFO(CAM_FLASH, "flash_name %s", flash_ctrl->flash_name);
	...
	mutex_init(&(vendor_flash_ctrl->ftm_flash_lock));
...
int shutdown_flash_notify(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	if (vendor_flash_ctrl->io_master_info.master_type == I2C_MASTER) {
		...
		vendor_flash_ctrl->io_master_info.cci_client->sid = cur_flash_ftm_data->cci_client.sid >> 1;
	}
	...
	if((cur_flash_ftm_data->need_standby_mode) && (flash_ftm_data.cur_flash_status != IIC_FLASH_OFF)) {
		rc = camera_io_dev_write(&(vendor_flash_ctrl->io_master_info), &write_setting);
		if (rc < 0) {
			CAM_ERR(CAM_FLASH, "Set flash off in shutdown failed!");
			rc = NOTIFY_DONE;
		} else {
			CAM_INFO(CAM_FLASH, "Set flash off in shutdown successful, cur action:%d", action);
			rc = NOTIFY_OK;
		}
	}
